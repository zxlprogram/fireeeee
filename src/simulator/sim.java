...
class Detector {
    ...
    void update(Obj[][][] map) {
        if (map[z][y][x].smoke > 0.2) danger = true;
...
class People {
    ...
    public void escape(int currentTick) {
        ...
        boolean inSmoke = space.building[z][y][x].smoke > 0.2;
        ...
        if (inSmoke) {
            ...
            for (int step = 0; step < speed; step++) {
                ...
                if (nextPos[0] != this.z) {
                    ...
                    if (space.building[z][y][x].smoke > 0.2) {
    ...
    private int[] maximizeSurvivalTime(Set<Integer> knownFires, Set<Integer> knownHazards) {
        ...
        while (!pq.isEmpty()) {
            ...
            for (int i = 0; i < 4; i++) {
                ...
                int weight = (nextObj instanceof Door) ? 15 : 1;
...
public class sim {
    ...
    static Stage findSmokeUpTarget(Stage origin) {
        ...
        while (cur != null) { if (cur.smoke < 0.2) candidate = cur; cur = cur.upfloor; }
        return candidate;
    }
    static Stage findSmokeDownTarget(Stage origin) {
        Stage cur = origin.downfloor;
        while (cur != null) { if (cur.smoke < 0.2) return cur; cur = cur.downfloor; }
        return null;
    }
        ...
        for (int z = 0; z < height; z++) {
            ...
            for (int y = 0; y < rows; y++) {
                ...
                for (int x = 0; x < cols; x++) {
                    ...
                    else if (cell.smoke >= 0.2)          sb.append("💨"); 
    ...
    static int[] run(boolean useSmart, int totalPeople) {
        ...
        for (int i = 0; i < totalPeople; i++) {
            if (survivalTimes[i] == 0) { // 0 代表活到了最後
    ...
    public static void main(String[]args) {
        ...
        s.work(1,new Range(1,2),new Range(5,20),new Range(5,20));
    }
