...
| **HYBRID** | **決策支援＋備援主動控制模式**：除了具備 SMART 模式的指引功能外，系統若偵測到危險（半徑 2 格內 `smoke > 0.5` 或網格著火），將自動觸發兩項主動實體控制：1. **強制關閉**該區域所有未關緊的防火門（`FIRE_DOOR`）。2. **啟動局部排煙**，使該受災區域的煙霧每步增長率乘上 **0.85** 的折減係數。
---
...
| 角色類型 | 移動速度(格/Tick) | 基準 CO 致命閾值 | 恐慌易感度 | 特殊行為邏輯與限制 |
| -- | -- | -- | -- | -- |
